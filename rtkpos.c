...
#define MAXACC      1.0     /* max accel for doppler slip detection (m/s^2) */
                     ...
                     const prcopt_t *opt)
{
    ...
    if (f>=nf&&opt->exterr.ena[0]) { /* code */
        ...
        // modified by Peng Xie, adjust R matrix
        if (f == 1)
        {
            fact = 100;
        }
                       ...
                       const nav_t *nav)
{
    ...
    // Opened by Peng Xie, 2/14/2018
    // clock-jump issue could be detected by some other algorithms
#if 1
    int j, f,sat=obs[i].sat;
    double tt,dph,dpt,lam,thres;
    double rr_[3], pos[3], acc;
    for (j = 0; j<3; j++) rr_[j] = rtk->sol.rr[j];

    acc = rtk->x[6] * rtk->x[6] + rtk->x[7] * rtk->x[7] + rtk->x[8] * rtk->x[8];
    acc = sqrt(acc);
    ...
    for (f=0;f<rtk->opt.nf;f++) {
        ...
        acc = max(acc, MAXACC);
        thres = acc*tt*tt / 2.0 / lam + rtk->opt.err[4] * fabs(tt)*4.0;
        ...
        fprintf(statesKF, "slip detected (sat=%2d rcv=%d L%d=%.3f %.3f thres=%.3f  phase=%.3f  diff=%.3f    %.6f   %.6f   time=%d )\n",
            sat, rcv, f + 1, dph, dpt, thres, obs[i].L[f], fabs(dph - dpt), pos[0]*180/3.1415926, pos[1] * 180 / 3.1415926, obs[i].time.time);
                   ...
                   const int *iu, const int *ir, int ns, const nav_t *nav)
{
    ...
    for (f=0;f<nf;f++) {
        ...
        for (i=0;i<ns;i++) {
            ...
            rtk->x[j]=0.0;
            //initx(rtk, 0.0, 0.0, IB(sat[i], f, &rtk->opt));
                      ...
                      const prcopt_t *opt, double *y)
{
    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* iono-free linear combination */
        ...
        for (i=0;i<nf;i++) {
            ...
            // modified by Peng Xie, output obs directly
            //if (obs->L[i] != 0.0) y[i] = obs->L[i] * lam[i];
            //if (obs->P[i] != 0.0) y[i + nf] = obs->P[i];
            //{
            //    //fprintf(statesKF, " %f,   ", v[nv]);
            //    fprintf(statesKF, " %f,  %f \n", obs->L[i] * lam[i], obs->P[i]);
            //}
                 ...
                 double *e, double *azel)
{
    ...
    for (i=0;i<n;i++) {
        ...
        // added by Peng Xie, output raw phase measurement
        //if ((obs + i)->sat == 14 || (obs + i)->sat == 10) {
        //    fprintf(statesKF, "   %d     %d    %f", (obs + i)->sat, (obs + i)->rcv, (obs + i)->L[0] * 0.1902);
        //}
    }
    //fprintf(statesKF, " \n");
                 ...
                 double *H, double *R, int *vflg)
{
    ...
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
        ...
        for (i=-1,j=0;j<ns;j++) {
            ...
            //added by Peng Xie, if cycle slip detected
        ...
        for (j=0;j<ns;j++) {
            ...
            if (H) Hi=H+nv*rtk->nx;
        // added by peng xie,
        if (rtk->sol.time.time == 1518899228)
        {
            v[2] = v[2];
            //v[2] = 0.0;
            //v[3] = 0.0;
            ...
            //// added by Peng Xie, output double difference measurement
            if (sat[j] == 14 && sat[i] == 10 && f == 0)
            {
                //fprintf(statesKF, " %f \n", v[nv]);
            }
            ...
            /************************************************************
            // modified by Peng Xie, reduce innovation threshold
            //opt->maxinno = 20.0;
            // test innovation 
            if (f<nf) opt->maxinno = 5;
            else      opt->maxinno = 0.5;

            if (f==0) opt->maxinno = 3.24*0.3;
            else      opt->maxinno = 3.24*10.0;
            ************************************************************/
            //if (f == 0) opt->maxinno = 3.24*1.0;
            //else      opt->maxinno = 3.24*10.0;
                  ...
                  const nav_t *nav)
{
    ...
    //// added by Peng Xie, for static data
    //if (obs[nu].time.time > 1511011365.8)
    //{
    //    dt = dt;
    //}

    // added by Peng Xie, for dynamic data
    if (obs[nu].time.time > 1512329990)
    {
        dt = dt;
    }


    // added by Peng Xie, output single difference iono estimation
    //if (sat[j] == 2 && sat[i] == 25 && f == 0)
    {
        //fprintf(statesKF, " %f    %f    %f    %f    %f    %f\n", rtk->x[10], rtk->x[12], rtk->x[16], rtk->x[20], rtk->x[23], rtk->x[111]);
        //fprintf(statesKF, " %f,   ", y[f + ir[j] * nf * 2]);
    }
    ...
    // added by peng xie,
    if (obs[nu].time.time == 1512329989)
    {
        n = n;
    }
    if (obs[nu].time.time == 1512329990)
    ...
    for (i=0;i<niter;i++) {
        ...
        //// added by Peng Xie, hold rover position
        //xp[0] = -2.680589702975841e+06;
        //xp[1] = -4.304940345405783e+06;
        //xp[2] = 3.855123259452441e+06;
        ...
        // added by peng xie,
        //if (obs[nu].time.time > 1518899227 && obs[nu].time.time < 1518899237)
        //{
        //    v[5] = 0.0;
        //    v[9] = 0.0;
        //}
        ...
        //rtk->x[0] = -2680755.1765940259;
        //rtk->x[1] = -4305050.1580890687;
        //rtk->x[2] = 3854899.1930595152;
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    //for (i = 0; i < n; i++) {
    //    // added by Peng Xie, output raw phase measurement
    //    if ((obs + i)->sat == 8 || (obs + i)->sat == 10) {
    //        fprintf(statesKF, "   %d     %d    %f", (obs + i)->sat, (obs + i)->rcv, (obs + i)->L[0] * 0.1902);
    //    }
    //}

    // added by Peng Xie, output float ambiguity
    // fprintf(statesKF, "   %f      %f", rtk->x[92], rtk->x[98]);
    //fprintf(statesKF, "   %f      %f      %f", rtk->x[12] - rtk->x[10], rtk->x[16] - rtk->x[10], rtk->x[20] - rtk->x[10]);
    if (epoch > 563)
    {
        epoch = epoch;
    }
    fprintf(statesKF, "   %f      %f      %f", rtk->x[93] - rtk->x[113], rtk->x[97] - rtk->x[113], rtk->x[99] - rtk->x[113]);
    fprintf(statesKF, "   \n");
    epoch++;
    return 1;
}
